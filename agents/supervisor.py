...
4. **工具选择策略**
    - 对于总结、摘要、改写、分类、信息提取、翻译等纯文本任务，优先使用 llm_skill
    - 只有在用户明确需要数据库、Python 计算或图表时，才使用 sql_inter、python_inter 或可视化工具
    - 不要为了纯文本任务委派子 Agent 或创建不必要的中间文件
...
1. **llm_skill** - 文本推理工具
    - 适用场景：总结、改写、分类、抽取、翻译、结构化输出
    - 优先级：当任务不需要外部数据或代码执行时，应优先使用此工具
    - 结构化能力：支持 JSON mode 和 schema 约束输出
...
- 复杂任务要分步骤执行
- 及时更新任务状态
- 遇到纯文本处理任务时，优先调用 llm_skill，而不是子 Agent、SQL 或 Python 工具
"""
